...
try:
    with open('.\\FILE_PATH.json') as path_file:
        ...
        if path_data["input_dir"] != "" and os.path.isdir(path_data["input_dir"]):
            ...
            input_path = os.path.join(cwd, "uploaded-docs")
...
# open uploaded-docs foler in explorer
subprocess.call('explorer {}'.format(input_path))

os.system('cls')
print("Place the idocs you would like formted in the 'uploaded-docs' folder.")
os.system('pause')
...
if len(file_list) == 0:
    ...
    print("Please remove the idocs from the 'uploaded-docs' folder.")
    os.system("pause")
